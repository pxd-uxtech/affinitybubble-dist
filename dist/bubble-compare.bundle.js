...
function guessCategoryKeys(cols, data, d3) {
  ...
  const excludeKeys = new Set([
    ...
    "bigCluster", "bigClusterOrder", "cluster_keywords", "x", "y", "cellPos",
    "size", "bubbleSize", "가중치"
  ]);
 ...
 * @param {Function} [options.onSelectorCreated] - selector div 생성 시 콜백 (el) => void
 * @param {Function} [options.onDateRadioCreated] - date radio div 생성 시 콜백 (el) => void
 *   페이지의 디자인 시스템(예: toggle-radio)에 통합할 때 사용. 인라인 스타일은
 *   사용하지 않으므로 class 추가/제거로 자유롭게 덮어쓸 수 있다.
 */
export function createBubbleCompare(container, clusterWithLabel, options = {}) {
  ...
  // 날짜 컬럼이 있으면 카테고리에서 같은 컬럼명 제외 (중복 표시 방지)
  const dateColNameSet = new Set(dateColumns.map(dc => dc.toLowerCase()));
  dateColNameSet.add("date");
  const filteredCategoryKeys = dateColumns.length > 0
    ? categoryKeys.filter(k => !dateColNameSet.has(k.toLowerCase()))
    : categoryKeys;
  ...
  // 날짜 라디오 버튼 컨테이너 (인라인 스타일 없이 class만 부여 — 페이지 디자인 시스템과 통합 가능)
  const dateRadioDiv = document.createElement("div");
  dateRadioDiv.className = "bubble-compare-date-radio";
  dateRadioDiv.hidden = true; // display:none 대신 [hidden] 속성 사용 (CSS override 가능)
  selectorDiv.appendChild(dateRadioDiv);

  // 페이지에서 selector / date radio를 디자인 시스템에 통합할 수 있도록 콜백 제공
  if (typeof onSelectorCreated === "function") {
    try { onSelectorCreated(selectorDiv); } catch (e) { console.warn("onSelectorCreated:", e); }
  }
  if (typeof onDateRadioCreated === "function") {
    try { onDateRadioCreated(dateRadioDiv); } catch (e) { console.warn("onDateRadioCreated:", e); }
  }
  ...
  // 날짜 라디오 구성 함수 (인라인 스타일 없음 — class로만 처리)
  function setupDateRadio(dateCol) {
    ...
    // 월별만 있으면 선택지가 하나뿐이므로 라디오 자체를 숨김 (헷갈림 방지)
    if (!hasWeekly) {
      dateRadioDiv.hidden = true;
      return;
    }
  ...
  function renderCharts(selKey) {
    ...
    bumpTitle.textContent = customTitle != null ? customTitle : `이슈 x [${selKey}] 교차 분석`;
    ...
    try {
      ...
      if (itemCompare.length > 0) {
        ...
        // regionColors가 있으면 bigLabel별 정확한 색상 매핑 (다른 차트와 색상 일치)
        ...
        if (regionColors && regionColors.length) {
          ...
          bumpColorRange = bigLabels.map((bl, i) =>
            rcMap.get(bl) || cardColors[i] || CLUSTER_COLORS[i]
          );
        }
    ...
    if (VTClass) {
      ...
      smTitle.textContent = customTitle != null ? customTitle : `이슈 x [${selKey}] Small Multiples`;
...
/**
 * 비교 가능한 컬럼 목록을 미리 조회
 * @param {Array} clusterWithLabel - 분석 결과 데이터
 * @returns {{ categoryKeys: string[], dateColumns: { key: string, monthSpan: number, weekSpan: number }[] }}
 */
function getCompareColumns(clusterWithLabel) {
  ...
  dateColNameSet.add("date");
  const filtered = dateColumns.length > 0
    ? categoryKeys.filter(k => !dateColNameSet.has(k.toLowerCase()))
    : categoryKeys;
