...
function createHierarchicalTable(data, bubbleData = [], options = {}) {
  ...
  const effectiveUserColumns = userColumns.length > 0 ? userColumns : data.length > 0 ? Object.keys(data[0]).filter((k) => !STANDARD_FIELDS.has(k)) : [];
  ...
  const headerTexts = {
    normal: ["\uC0C1\uC704 \uD074\uB7EC\uC2A4\uD130", "\uD558\uC704 \uD074\uB7EC\uC2A4\uD130", "\uD14D\uC2A4\uD2B8", "Size", ...effectiveUserColumns],
    userGoal: ["User Goal", "User Needs", "User Voice", "Size", ...effectiveUserColumns]
  };
  ...
  function generateTSV() {
    ...
    if (isCompact) {
      ...
      data.forEach((curr) => {
        ...
        if (!group) {
          ...
          effectiveUserColumns.forEach((col) => {
    ...
    processedData.forEach((item) => {
      const userColumnValues = effectiveUserColumns.map((col) => item[col] || "");
  ...
  function renderTableBody() {
    ...
    const structuredData = processedData.reduce((acc, curr) => {
      ...
      acc[curr.bigLabel][curr.label].push({
        ...
        size: curr.size,
        cluster: curr.cluster,
        userColumns: effectiveUserColumns.reduce((obj, col) => {
          obj[col] = curr[col];
          return obj;
        }, {})
      });
    ...
    sortedEntries.forEach(([bigLabel, labels]) => {
      ...
      Object.entries(labels).forEach(([label, chunks]) => {
        ...
        chunks.forEach((chunkData) => {
          ...
          const userColumnCells = effectiveUserColumns.map((col) => {
  ...
  const additionalHeaders = effectiveUserColumns.map((column) => createHeaderCell(column, 80));
