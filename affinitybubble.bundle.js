...
function createChunkData(rawData, columnMapping, options = {}) {
  const { maxSize = 1e3, reservoirSample = null, userSubscript = "FREE" } = options;
  const filtered = rawData.filter(
    (d) => d[columnMapping.text]?.replace(/\\n/g, "\n")
  );
  let samples;
  if (userSubscript.match(/free|basic/i) || !reservoirSample) {
    samples = filtered.slice(0, maxSize);
  } else {
    samples = reservoirSample(filtered, maxSize)[0];
  }
...
var Level1Pipeline = class {
  ...
  async run(chunkData, onProgress = () => {
    ...
    const embedResult = await this.doEmbedding(chunkData, (p) => {
      onProgress({ stage: "embedding", progress: p.progress, partialResult: p.embeds });
    });
  ...
  async _embedWithSampling(chunkData, onProgress) {
    ...
    const sampleWithEmbeds = sample.map((d, i) => ({ ...d, embed: sampleEmbeds[i] }));
    const restResult = await this._embedAndAssignRest(
      rest,
      sampleWithEmbeds,
      assignThreshold,
      (p) => onProgress({ progress: 30 + p * 0.7, embeds: p.allEmbeds })
      // 30-100%
    );
    return {
  ...
  /**
   * 나머지 데이터 임베딩 및 클러스터 배정
   */
  async _embedAndAssignRest(rest, initialEmbeds, threshold, onProgress) {
    const batchSize = 300;
    const allEmbeds = [];
    const centroids = this._computeCentroids(initialEmbeds);
    for (let i = 0; i < rest.length; i += batchSize) {
      ...
      for (let j = 0; j < batch.length; j++) {
        const withEmbed = { ...batch[j], embed: batchEmbeds[j] };
...
var AffinityBubblePipeline = class {
  ...
  async run(chunkData, options = {}, onProgress = () => {
    ...
    try {
      ...
      const level1Result = await this.level1.run(chunkData, (p) => {
      ...
      const labels = await this._doLabeling(
        clustersForLabeling,
      ...
      const labelClusters = await this._rearrangeOutliers(
        labels,
      ...
      const level2Result = await this.level2.run(
        labelClusters,
        // Pass the rearranged clusters!
        { selUsecase, bigLabelOption, selLabelLanguage },
        (p) => {
  ...
  _determineStartStage(chunkData, options) {
    if (!this.prevOptions || this.prevChunkDataLength !== chunkData.length) {
      return "level1";
    }
    if (this.prevOptions.labelOption !== options.labelOption || this.prevOptions.selLabelLanguage !== options.selLabelLanguage) {
      return "labeling";
    }
    if (this.prevOptions.bigLabelOption !== options.bigLabelOption || this.prevOptions.selUsecase !== options.selUsecase) {
      return "level2";
    }
    return "level1";
  }
